...
class RemoteSliderWindow:
    ...
    def show_ui(self) -> str:
        ...
        cmds.formLayout(main_layout, edit=True,
                        attachForm=[
                            (node_list, 'top', 5), (node_list, 'left', 5), (node_list, 'right', 5),
                            (slider, 'left', 5), (slider, 'right', 5),
                            (attr_list, 'left', 5), (attr_list, 'right', 5), (attr_list, 'bottom', 5)
                        ],
                        attachControl=[
        ...
        for i, field in zip([-1., 1.], [self.min_field, self.max_field]):
    ...
    def change_slider_value(self, value, *args):
        """Change slider value.
    ...
    def changed_slider_value(self, value, *args):
    ...
    def change_field_value(self, value, *args):
        """Change field value.
    ...
    def _change_value(self, value: float, mode: str, node_attributes: list[str], *args):
        """Change value.
    ...
    def step_value(self, step_value, *args):
        ...
        if mode == 'local_relative':
            for node_attributes in node_attributes:
                current_value = cmds.getAttr(node_attributes)
            ...
            for node_attributes in node_attributes:
                ...
                cmds.setAttr(node_attributes, current_slider_value + step_value)
    ...
    def _get_nodes(self, selected: bool = True) -> list[str]:
        ...
        for item in nodes:
            if cmds.objExists(item):
                ...
                cmds.warning('Node does not exist: {}'.format(item))
        ...
        if mode == 'world_relative':
                ...
                if attribute not in ['translateX', 'translateY', 'translateZ', 'rotateX', 'rotateY', 'rotateZ']:
                    ...
                    for i, node_attribute in enumerate(node_attributes):
                        ...
                        if not lib_attribute.is_modifiable(node, parent_attribute):
                            cmds.warning('Attribute is not modifiable: This attribute or its sibling attributes: {}'.format(node_attribute))
                            status = False
            ...
            for node_attribute in node_attributes:
                ...
                if not lib_attribute.is_modifiable(node, attribute):
                    cmds.warning('Attribute is not modifiable: {}'.format(node_attribute))
                    status = False
...
class DagNodeValues:
    ...
    def add_node(self, node: str):
        ...
        if not cmds.objExists(node):
            cmds.error('Node does not exist: {}'.format(node))

        if self.has_node(node):
            cmds.warning('Node already added to data: {}'.format(node))
            return
    ...
    def remove_node(self, node: str):
        ...
        if not self.has_node(node):
            cmds.warning('Node does not exist in data: {}'.format(node))
            return
