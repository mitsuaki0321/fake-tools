...
class MainWindow(base_window.BaseMainWindow):
    ...
    def same_selection(self):
        ...
        if not cmds.nodeType(mesh_transform) == 'transform':
        ...
        if not cmds.nodeType(mesh[0]) == 'mesh':
    ...
    def uv_area_selection(self):
        ...
        if self.uv_button_group.checkedButton().text() == 'u':
            uv = 'u'
        else:
            uv = 'v'

        uv_components = lib_component.ComponentSelection(sel_components).uv_area_selection(uv=uv,
                                                                                           area=[min_param, max_param])
    ...
    def __select_objects_to_components(self, filter_types: list[str] = ['mesh', 'nurbsCurve',
                                                                        'nurbsSurface', 'lattice']) -> list[str]:
    ...
    def __update_uv_spinbox(self):
        ...
        elif sender == self.min_param_spinbox:
